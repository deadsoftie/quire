...
- **Root targeting.** `CompileRequest.targetRoot: DocUri | null` and
  `CompileResponse.root: DocUri` were added so a user can pick which `.tex` file
  compiles into the preview instead of always trusting automatic detection
  (`project::detect_root`). `quire-core` holds no state, so the choice has to travel
  with every `compile()` call the same way `engine` already does, rather than being
