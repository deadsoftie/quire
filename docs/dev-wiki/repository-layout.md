...
QUIRE_SPEC.md          The original design spec
M4_TASKS.md            Current milestone's task-by-task implementation notes
```
...
The split also matters for the future: D5 in `QUIRE_SPEC.md` requires an eventual iPad
build (Capacitor), which can't spawn subprocesses. A pure library with no process model
