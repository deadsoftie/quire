...
An index, not a copy, into the project's spec and current milestone notes - both are
actively maintained and this page will go stale the moment it tries to restate their
content instead of pointing at it.
...
| M3 - Own completion | quire-core's own indexing/completion; the GPL-3.0 scaffolding from M0/M1 removed |
...
| M4 - Ship desktop | Current milestone |
...
incompatible with App Store distribution terms) - the spec names it and has the full
licensing table. CI fails if its name is ever reintroduced anywhere in the tracked
codebase (deliberately not spelled out on this page, for that reason).
...
Implementation notes for the current milestone are captured as work lands - not just
"what," but the real reasoning behind non-obvious decisions, discovered gaps, and what
was deliberately deferred. That detail is worth reading directly rather than
summarized here; it's written for exactly this purpose. Those notes are removed once
the whole milestone is manually verified - check the milestones table above if they're
gone by the time you're reading this.
...
- **Precompiled-preamble caching is permanently blocked**, confirmed against Tectonic's
  actual engine, not a workaround-pending situation - this has been investigated in
  full, so it isn't worth budgeting time to attempt again.
